...
+ (NSMutableArray *)fitnessFuncWithSS:(NSMutableArray *)SSs andChromosome:(NSString *)chromosome andRecognitionRatio:(float)ratio {
    float fitness = .0f;
    NSMutableArray *numArray = [UtilityFunc numberOfAmbiguityAndActivatedBSWithSS:SSs andChromosome:chromosome];
    int numOfActivatedBS = [[numArray objectAtIndex:0] intValue];
    int numOfAmbiguity = [[numArray objectAtIndex:1] intValue];
    fitness = (1-ratio) * (float)numOfActivatedBS/NumberOfPotentialBS + ratio * (float)numOfAmbiguity/19900;
    [numArray addObject:[NSString stringWithFormat:@"%f", fitness]];
    
    return numArray;
}

+ (NSMutableArray *)numberOfAmbiguityAndActivatedBSWithSS:(NSMutableArray *)SSs andChromosome:(NSString *)chromosome {
    int numOfActivatedBS = 0;
    int numOfAmbiguity = 0;
    NSMutableArray *numArray = [[NSMutableArray alloc] init];
    /**
    ...
    @autoreleasepool {
        ...
        for (int i = 0; i < NumberOfPotentialBS; i++) {
            if ([[chromosome substringWithRange:NSMakeRange(i, 1)] boolValue]) {
                numOfActivatedBS += 1;
                [activatedIndexes addObject:[NSNumber numberWithInt:i]];
            }
        }
    ...
    [numArray addObject:[NSString stringWithFormat:@"%d", numOfActivatedBS]];
    [numArray addObject:[NSString stringWithFormat:@"%d", numOfAmbiguity]];
